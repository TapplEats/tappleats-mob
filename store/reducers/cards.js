...
const getCardSuccess = (state, action) => updateObj(state, {
  ...
  offersOrder: action.offersOrder,
  offersSettings: action.offersSettings,
  offerTags: action.offerTags,
...
const addOffer = (state, action) => {
  const newOffers = state.offers ? [...state.offers, action.offerObj] : [action.offerObj]
...
const removeOffer = (state, action) => {
  const newOffers = [...state.offers].filter(offer => {
    const storeOfferDate = offer.addedOn.seconds ? offer.addedOn.toDate() : offer.addedOn
    const selectedOfferDate = action.addedOn.seconds ? action.addedOn.toDate() : action.addedOn
    return !isEqual(storeOfferDate, selectedOfferDate)
  })
  return updateObj(state, {
    offers: newOffers,
    loading: false,
...
const updateOffer = (state, action) => {
  ...
  const offerIndex = currentOffers.findIndex(offer => {
    const stateOfferDate = offer.addedOn.seconds ? offer.addedOn.toDate() : offer.addedOn
    const actionOfferDate = action.addedOn.seconds ? action.addedOn.toDate() : action.addedOn
    return isEqual(stateOfferDate, actionOfferDate)
  })
  currentOffers[offerIndex].image = action.image
  currentOffers[offerIndex].title = action.title
  currentOffers[offerIndex].description = action.description
  currentOffers[offerIndex].tagLine = action.tagLine
  currentOffers[offerIndex].startDate = action.startDate
  currentOffers[offerIndex].endDate = action.endDate
  currentOffers[offerIndex].onlyOn = action.onlyOn
  currentOffers[offerIndex].limit = action.limit
  currentOffers[offerIndex].oldPrice = action.oldPrice
  currentOffers[offerIndex].newPrice = action.newPrice
  currentOffers[offerIndex].discount = action.discount
  currentOffers[offerIndex].active = action.active
  currentOffers[offerIndex].tags = action.tags
...
const sortOffers = (state, action) => {
  ...
  const { sortType } = action
  let newOffers = null
  if (state.offersOrder) {
    newOffers = [...state.offersOrder].sort((a, b) => {
      let arg1
      let arg2
      const aDate = a.addedOn.seconds ? a.addedOn.toDate() : a.addedOn
      const bDate = b.addedOn.seconds ? b.addedOn.toDate() : b.addedOn
...
// const updateSortedOffers = (state, action) => {
//   const offersOrder = [...state.offersOrder]
//   return updateObj(state, {
//     offersOrder: arrayMove(offersOrder, action.oldIndex, action.newIndex),
//     changed: true,
//   })
// }
const updateOfferLimit = (state, action) => {
  const currentOffers = [
    ...state.offers,
  ]
  const offerIndex = currentOffers.findIndex(offer => offer.offerId === action.offerId)
  currentOffers[offerIndex].limit -= 1
...
const addTag = (state, action) => {
  const newTags = state.offerTags ? [...state.offerTags, action.tagObj] : [action.tagObj]
...
const cardReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case actionTypes.SORT_CARD_OFFERS: return sortOffers(state, action)
    case actionTypes.ADD_CARD_OFFER: return addOffer(state, action)
    case actionTypes.REMOVE_CARD_OFFER: return removeOffer(state, action)
    case actionTypes.UPDATE_CARD_OFFER: return updateOffer(state, action)
    case actionTypes.UPDATE_SORTED_CARD_OFFER: return updateSortedOffers(state, action)
    case actionTypes.UPDATE_OFFER_LIMIT: return updateOfferLimit(state, action)
    case actionTypes.ADD_OFFER_TAG: return addTag(state, action)
